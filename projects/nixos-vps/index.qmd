...
- name: Git checkout
  ansible.builtin.git:
    repo: 'https://foosball.example.'
    dest: /home/moonpie
    version: release-0.22 # can be branch, tag, or sha1 hashshum of the repo.

```

In addition to that, I also need a way to keep the system updated, for security purposes. Because I am using ubuntu, I will use the ansible apt modele
...
- name: update the distrobution itself? Still working on this one
  ansible.builtin.apt:
