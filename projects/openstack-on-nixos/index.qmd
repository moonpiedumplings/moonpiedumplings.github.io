...
o this is good advice. I just graduated, and although I have an internship I will need to do this anix-shell
error:
...
A user on matrix [responded to me asking for help with dream2nix](https://matrix.to/#/!PrIEtcffTLLzUWvJOS:matrix.org/$0qTdfRYYUTrx4eHQuIxdcegjHyauFIsX5ZsTMiXRA44?via=nixos.org&via=matrix.org&via=nixos.dev)
...
```{.nix .code-overflow-wrap filename="keystone.nix"}
{
...
I found something that worked. Looking at the old [nixstack code](https://github.com/hyphon81/Nixtack/blob/master/openstack/keystone/keystone.nix), there is an attribute that can be set:
...
{backticks}{{.nix filename="shell.nix"}}
{open('mach-nix/shell.nix', 'r').read()}
{backticks}

{backticks}{{.nix filename="keystone.nix"}}
{open('mach-nix/keystone.nix','r').read()}
{backticks}
...
So I started doing some research. The first thing was something another user on matrix sent to me, [a stackoverflow post](https://stackoverflow.com/questions/58854822/python-setup-py-sdist-error-versioning-for-this-project-requires-either-an-sdist)
...
One [stackoverflow](https://stackoverflow.com/questions/75878318/after-upgrade-getting-error-repository-not-owned-by-current-user) answer recommended this:
...
Okay, so on the [matrix thread where I am being helped](https://matrix.to/#/!PrIEtcffTLLzUWvJOS:matrix.org/$aYf16rjciA7xC0Ac56DUNMyvx58eW1uBd8vG594hVRQ?via=nixos.org&via=matrix.org&via=nixos.dev), I received this as a reply when I asked how to set up environment variables in dream2nix: 
...
Poetry is a python dependency manager similar to nix. [poetry2nix](https://github.com/nix-community/poetry2nix) uses poetry's specifications to do things like download package, package applications, or create shell environments using nix. 

First I needed to convert the requirements.txt to poetry.

I used the [dephell](https://github.com/dephell/dephell) tool to do so. Specifically, [`dephell deps convert`](https://dephell.readthedocs.io/cmd-deps-convert.html)
...
```{.nix .code-overflow-wrap filename="shell.nix"}
{ 
pkgs ? import <nixpkgs> {},
...
| Project | Notes |
| - | - |
| [dephell](https://github.com/dephell/dephell) | Works, but is archive/unmaintained, and complains about a piece being deprecated when ran... |
| [stanza](https://github.com/src-r-r/python-stanza) | Last commit was a year ago|
| [poeareq](https://github.com/tddschn/poetry-add-requirements.txt) | maintained by one person, last commit a year ago. Present in pypi |
| [tigerhawk's gist](https://gist.github.com/tigerhawkvok/856bc4690353e14a6ee03db20c0dfbd4) | just a github gist, a user complimented in a comment one month ago. But the script was last updated in 2019 |
| [various shell one liners](https://stackoverflow.com/questions/62764148/how-to-import-requirements-txt-from-an-existing-project-using-poetry/72456245#72456245) | provided by stackoverflow. They work, but are hacky, and apparently have edge cases they don't cover. |
| [pip-poetry-migration](https://github.com/expobrain/pip-poetry-migration) | untested/unresearched |
| [poetry-import](https://github.com/BrandonLWhite/poetry-import) | untested/unresearched. In pypi. |
| [poetrify](https://github.com/kk6/poetrify) | last updated 2 years ago. Says that requirements.txt is still in trial in description. |
| [poetry import plugin](https://github.com/neriberto/poetry-plugin-import) | poetry plugin, so it may be easier to install/automate. Last updated 9 months ago. |
| [micropipenv](https://github.com/thoth-station/micropipenv) | I don't know if it can actually do what I want. last updated 2 months ago. Maintained by Red Hat. |
...
{backticks}{{.nix filename="shell.nix"}}
{open('poetry/shell.nix', 'r').read()}
{backticks}

{backticks}{{.nix filename="keystone.nix"}}
{open('poetry/keystone.nix','r').read()}
{backticks}
