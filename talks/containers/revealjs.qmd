...
* Many groups needed containers in mainline, so they contributed
    - namespaces, cgroups (google), and 
* Eventually, LXC was the first iteration of modern container software
    - Docker was based of off lxc, but eventually switched to their own runtime


## Pet vs Application Type
...
* Simple
* Runs as root
* One daaemon, that autostarts, and handles containers as services
* Docker socket (/var/run/docker.sock), which can be served over the network
* Downside: runs as root, having access to the docker socket means you have root privileges
...
* Hashicorp Nomad
    ...
    - supports the most runtimes out of any other option
...
* You are a researcher at the SCP 
